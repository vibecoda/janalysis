...
from jqsys.portfolio import Market, Portfolio
from jqsys.stock import InsufficientDataError, Stock, StockNotFoundError
...
def demo_market_api():
    ...
    try:
        ...
        if len(volatile_stocks) > 0:
            ...
            print(
                volatile_stocks.select(
                    ["code", "avg_price", "annualized_volatility", "trading_days"]
                )
            )
        else:
...
def demo_stock_comparison():
    ...
    try:
        ...
        print(
            f"Stock 1 ({stock1.code}) - Total Return: {perf1['total_return']:.2%}, Volatility: {perf1['volatility']:.2%}"
        )
        print(
            f"Stock 2 ({stock2.code}) - Total Return: {perf2['total_return']:.2%}, Volatility: {perf2['volatility']:.2%}"
        )
        ...
        print("\nVolume Comparison:")
        print(
            f"Stock 1 ({stock1.code}) - Avg Volume: {vol1['avg_volume']:,.0f}, VWAP: ¥{vol1['vwap']:.2f}"
        )
        print(
            f"Stock 2 ({stock2.code}) - Avg Volume: {vol2['avg_volume']:,.0f}, VWAP: ¥{vol2['vwap']:.2f}"
        )
