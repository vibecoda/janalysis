...
def main() -> int:
    ...
    parser.add_argument(
        "--date", type=str, help="Date to ingest (YYYYMMDD format). Defaults to yesterday."
    )
    parser.add_argument(
        "--from", dest="from_date", type=str, help="Start date for range ingestion (YYYYMMDD)"
    )
    parser.add_argument(
        "--to", dest="to_date", type=str, help="End date for range ingestion (YYYYMMDD)"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force re-ingestion even if data already exists"
    )
    parser.add_argument("--query", action="store_true", help="Run sample queries after ingestion")
