...
class QueryEngine:
    ...
    def __init__(
        ...
        self.bronze_path = Path(bronze_path) if bronze_path is not None else data_root_path / "bronze"
        self.silver_path = Path(silver_path) if silver_path is not None else data_root_path / "silver"
    ...
    def calculate_returns(
        self,
        codes: Optional[List[str]] = None,
        periods: List[int] = [1, 5, 21]  # 1D, 1W, 1M
    ) -> pl.DataFrame:
        ...
        for period in periods:
            ...
            lag_expressions.append(f"LAG(close, {period}) OVER (PARTITION BY code ORDER BY date) as {lag_col}")
