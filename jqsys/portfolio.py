...
from .stock import Stock, InsufficientDataError
...
class Portfolio:
    ...
    def calculate_returns(
        ...
        for i, returns_df in enumerate(all_returns[1:], 1):
            merged_returns = merged_returns.join(
                returns_df.select(["date"] + [col for col in returns_df.columns if col.startswith("return_")]),
                on="date",
        ...
        for period in periods:
            return_cols = [col for col in merged_returns.columns if col == f"return_{period}d" or col.endswith(f"_{period}d")]
            ...
            if return_cols:
                ...
                for i, col in enumerate(return_cols):
                    if i < len(self.weights):
                        weighted_return_expr = weighted_return_expr + (pl.col(col).fill_null(0) * self.weights[i])
    ...
    def correlation_matrix(
        ...
        for stock in self.stocks:
            ...
            if not stock_returns.is_empty():
                stock_returns = stock_returns.select(["date", "return_1d"]).rename({"return_1d": stock.code})
                all_returns.append(stock_returns)
        ...
        for code1 in stock_codes:
            ...
            for code2 in stock_codes:
                if code1 == code2:
                    ...
                    corr = merged_returns.select(
                        pl.corr(code1, code2).alias("correlation")
                    )["correlation"][0]
    ...
    def performance_summary(
        ...
        return pl.DataFrame(performance_data).select([
            "code", "weight", "current_price", "total_return", "annualized_return",
            "volatility", "sharpe_ratio", "max_drawdown", "win_rate", "trading_days"
        ])
    ...
    def risk_metrics(
        ...
        returns_series = portfolio_returns.filter(
            pl.col("portfolio_return_1d").is_not_null()
        )["portfolio_return_1d"]
    ...
    def market_statistics(
        ...
        with QueryEngine() as query:
            ...
            if len(market_stats) == 0:
                raise InsufficientDataError(f"No market data available for {date.strftime('%Y-%m-%d')}")
