...
class Stock:
    ...
    def __init__(
        ...
        silver_storage: SilverStorage | None = None,
    ...
    def _validate_stock_exists(self) -> None:
        ...
        try:
            ...
            result = query.execute_sql_with_params(
                "SELECT COUNT(*) as count FROM daily_prices WHERE code = ?", [self.code]
            )
            if result["count"][0] == 0:
    ...
    def prices(
        self, start_date: str | datetime, end_date: str | datetime, columns: list[str] | None = None
    ) -> pl.DataFrame:
    ...
    def volatility_metrics(
        ...
        current_vol = (
            rolling_vol.drop_nulls().tail(1).item()
            if len(rolling_vol.drop_nulls()) > 0
            else daily_vol
        )
        ...
        return {
            ...
            "volatility_of_volatility": rolling_vol.drop_nulls().std()
            if len(rolling_vol.drop_nulls()) > 1
            else 0.0,
        }
    ...
    def correlation_with(
        ...
        # Merge on date
        merged = self_returns.join(other_returns, on="date", suffix="_other").filter(
            pl.col("return_1d").is_not_null() & pl.col("return_1d_other").is_not_null()
        )
    ...
    @property
    def data_range(self) -> dict[str, str] | None:
    ...
    def avg_volume(self) -> float | None:
        ...
        try:
            query = self._get_query_engine()
            result = query.execute_sql_with_params(
                "SELECT AVG(volume) as avg_vol FROM daily_prices WHERE code = ?", [self.code]
            )
